% BCG_Correction_PROJIC_OBS() - perform BCG artefact correction by correction the 
%                               BCG-related ICs selected by PROJIC for the artefact occurrences using
%                               OBS [Niazy et al., NeuroImage 2005], before back-projecting all the ICs in the reconstruction of the EEG signal
% Usage:
...
%   npc         - number of principal components (can be an array, and BCG correction 
%                 will be performed for all number of PCs specified, using OBS [Niazy et al., 2005])
...
%                 selection using PROJIC.m) -> if npc > 1 and k_clusters > 1, 
...
%                          representative windows. It has (npc, k_clusters + 1) elements, the first
...
function [ PowerFreq, PowerFreq_bkg ] = BCG_Correction_PROJIC_OBS(dataset, ...
...
PowerFreq = cell(3, np, nk + 1); % 1->power, 2->param, 3->ICs
PowerFreq_bkg = cell(3, np, nk + 1); % 1->power, 2->param, 3->ICs
...
for n = 1:np
    for k = 1:(nk + 1)
        ...
        if k < 2 % quantification before correction
        ...
        else % quantification after correction, for different number of clusters and principal components
            ...
            act_corrected = fmrib_pas_EEGActivations(activations, sort(ICs), R, 'obs', npc(n), [], Fs, delay_QRS);
            act_copy(sort(ICs), :) = act_corrected; % correct the selected ICs for artefact occurrences using OBS
            
            eeg_bcg = unmix_matrix * act_copy; param = [ k_clusters(k - 1) npc(n) ]; % back-reconstruct the EEG signal using all the ICs, including the OBS-corrected ones
        end
